...
In order to select models we use the function `select_models` which will select models with a max VIF of less than 5 and a delta AICc of less than a parameter called delta_aicc which defaults to 2 if we use the last model we used in the section above that would get us to:
