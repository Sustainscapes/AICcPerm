...
#' @param threshold A numeric value specifying the threshold for filtering models based on maximum VIF (default is 5)
...
filter_vif <- function(all_forms,
  ...
  if (filter) {
    Fs <- Fs |>
      dplyr::filter(max_vif < threshold)
  }

  if (!filter) {
    Fs <- Fs |>
      dplyr::mutate(collinearity = ifelse(max_vif < threshold, "low", "high"))
  }
